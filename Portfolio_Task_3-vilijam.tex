...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{casedata\_task3 }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"Task 3}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{CaseData\_Task3.csv"}\NormalTok{, }\AttributeTok{stringsAsFactors =} \ConstantTok{FALSE}\NormalTok{)}
...
\subsection{Estimate the treatment effect using simple linear Regression
...
\pandocbounded{\includegraphics[keepaspectratio]{Portfolio_Task_3-vilijam_files/figure-pdf/unnamed-chunk-5-1.pdf}}
...
\subsection{Matching comparisons}\label{matching-comparisons}
...
\subsubsection{1:1 Nearest Neighbor Matching WITHOUT
...
\FunctionTok{cat}\NormalTok{(}\StringTok{"Balance Summary (Without Replacement):}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{1:1 Nearest Neighbor Matching WITH
...
\FunctionTok{summary}\NormalTok{(model\_simple\_matched)}
\end{Highlighting}
\end{Shaded}
...
the doubly robust version (-10.3 vs.~23.3), which suggests that the
matching alone didn't achieve good balance and the regression adjustment
is still doing important work to control for confounding.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(lm\_euc)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Robust Standard Errors (Euclidean Matching):}
\FunctionTok{print}\NormalTok{(}\FunctionTok{coeftest}\NormalTok{(lm\_euc, }\AttributeTok{vcov. =} \FunctionTok{vcovHC}\NormalTok{(lm\_euc, }\AttributeTok{type =} \StringTok{"HC3"}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\textbf{Comparing matching quality across
...
\FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{Manual IPW ATE (primary estimate):"}\NormalTok{, }\FunctionTok{round}\NormalTok{(ate\_ipw\_manual, }\DecValTok{4}\NormalTok{), }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{print}\NormalTok{(}\FunctionTok{summary}\NormalTok{(lm\_ipw))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{print}\NormalTok{(}\FunctionTok{coeftest}\NormalTok{(lm\_ipw, }\AttributeTok{vcov. =} \FunctionTok{vcovHC}\NormalTok{(lm\_ipw, }\AttributeTok{type =} \StringTok{"HC3"}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{IPW Critical Discussion
Points}\label{ipw-critical-discussion-points}

The manual IPW estimate came out at around −318, which obviously doesn't
make sense for a sales effect and is likely due to its sensitivity to
poor overlap. The weighted regression version (\textasciitilde15.81)
does much better because the regression component doesn't rely purely on
the weights. It also shows why AIPW and DML are generally more
appealing: they don't put all their eggs in the weighting basket.

\subsection{Augmented Inverse Probability Weighting (AIPW) - Manual
...
\FunctionTok{cat}\NormalTok{(}\StringTok{"Manual AIPW ATE estimate:"}\NormalTok{, }\FunctionTok{round}\NormalTok{(aipw\_ate, }\DecValTok{4}\NormalTok{), }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{AIPW Critical Discussion
...
\subsection{Double Machine Learning (DML): PLR with
...
\subsubsection{DML Discussion Points}\label{dml-discussion-points}
...
\subsection{Comparison of Treatment Effects Across
...
\subsubsection{1. Comparison of ATE vs.~ATT
...
\subsubsection{2. Methodological Divergence and
...
\item
  ...
  weights. The weighted regression IPW (\textasciitilde15.81) is much
  more reasonable since it doesn't rely only on the weights.
\item
  \textbf{Matching differences:} The without-replacement estimate
  (17.15) and with-replacement estimate (23.25) are pretty far apart.
  Given the balance diagnostics, we'd trust the with-replacement version
  more -- the without-replacement matching left large SMDs on X10--X14,
  so those matched pairs weren't really comparable. The with-replacement
  version got balance mostly under control, though it comes with more
  variance from reusing controls.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{3. The Role of Double Robustness and Machine
...
\subsubsection{4. Conclusion: Which Estimate to
...
\textbf{Suggestions for the Marketing
...
\subsection{Reflection on AI Use}\label{reflection-on-ai-use}
