...
df <- df %>%
  mutate(
    ...
    # 'treated' is the actual interaction / DiD term 
    # (1 ONLY for Store 109 AND Weekind >= 78)
...
trend_data_p1 <- df %>%
  ...
  summarise(mean_sales = mean(p1sales, na.rm = TRUE),
            .groups = 'drop') %>%
  mutate(Group = ifelse(is_treated_store == 1, 
                        "Treated (Store 109)", 
                        "Control (Other Stores)"))

plot_p1 <- ggplot(trend_data_p1,
                  aes(x = Weekind, y = mean_sales, color = Group)) +
           ...
           annotate("text", 
                    x = 76, 
                    y = max(trend_data_p1$mean_sales), 
...
plot_p2 <- ggplot(trend_data_p2, aes(x = Weekind,
                                     y = mean_sales, 
                                     color = Group)) +
           geom_line(linewidth = 1) +
           geom_vline(xintercept = 78,
           ...
           annotate("text", 
                    x = 76, 
                    y = max(trend_data_p2$mean_sales), 
                    label = "Treatment Start", 
                    angle = 90, 
                    vjust = 1.5, 
                    color = "red") +
...
# Ensure data frame is standard (synthdid prefers plain data.frames over tibbles)
df <- as.data.frame(df)
...
# Extract synthetic counterfactual + RMSPE + donor weights from an SC estimate
sc_diagnostics <- function(sc_est, setup) {
  ...
  # SC donor weights: synthetic counterfactual is weighted avg of controls
...
coeftest(did_cov_p1,
         vcov = vcovHC(did_cov_p1,
         type = "HC1", 
         cluster = "group"))["treated", , drop = FALSE]
```
...
model_p1 <- lm(p1sales ~ p1price + p1prom + compind + storesize + citysize,
               data = df)
...
sdid_est_p1_adj <- synthdid_estimate(setup_p1_adj$Y,
                                     setup_p1_adj$N0,
                                     setup_p1_adj$T0)
...
# Run a linear model and extract the residuals (unexplained sales)
model_p2 <- lm(p2sales ~ p2price + p2prom + compind + storesize + citysize,
               data = df)
...
# Estimate the adjusted SDID Treatment Effect
sdid_est_p2_adj <- synthdid_estimate(setup_p2_adj$Y,
                                     setup_p2_adj$N0,
                                      setup_p2_adj$T0)
...
robust_twfe_p1 <- coeftest(twfe_p1,
                           vcov = vcovHC(twfe_p1,
                                         type = "HC1",
                                         cluster = "group"))
robust_twfe_p2 <- coeftest(twfe_p2,
                           vcov = vcovHC(twfe_p2,
                                         type = "HC1",
                                         cluster = "group"))

robust_did_cov_p1 <- coeftest(did_cov_p1,
                              vcov = vcovHC(did_cov_p1,
                                            type = "HC1",
                                            cluster = "group"))
robust_did_cov_p2 <- coeftest(did_cov_p2,
                              vcov = vcovHC(did_cov_p2,
                                            type = "HC1",
                                            cluster = "group"))
