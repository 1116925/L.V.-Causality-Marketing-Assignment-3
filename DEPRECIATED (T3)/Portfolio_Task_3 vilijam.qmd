...
coeftest(lm_euc, vcov. = vcovHC(lm_euc, type = "HC3"))["d", , drop = FALSE]
```
...
cat(sprintf("Bootstrap mean (IPW): $%.2f\n", ipw_boot_mean))
cat("Bootstrap SE (IPW):", round(ipw_boot_se, 4), "\n")
cat(sprintf("95%% percentile CI (IPW): [$%.2f, $%.2f]\n", ipw_boot_ci[1], ipw_boot_ci[2]))
...
# Secondary comparison: weighted lm IPW-RA model — treatment row only
...
cat(sprintf("Bootstrap mean (AIPW): $%.2f\n", aipw_boot_mean))
cat("Bootstrap SE (AIPW):", round(aipw_se, 4), "\n")
cat(sprintf("95%% Bootstrap CI (AIPW): [$%.2f, $%.2f]\n", aipw_ci[1], aipw_ci[2]))
```
