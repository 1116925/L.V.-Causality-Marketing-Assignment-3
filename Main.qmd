---
title: "L.V. Causality Marketing Assignment 3"
date: "`r Sys.Date()`"
format: 
  html:
...
A fundamental distinction in these findings is the estimand being measured.

* **Average Treatment Effect (ATE):** Regression Adjustment, IPW, AIPW, and Double ML all target the ATE, which is the average effect of the discount if it were applied to the entire population. These estimates (excluding the unstable manual IPW) range from approximately **13.70 to 18.85**.
* **Average Treatment Effect on the Treated (ATT):** Matching methods estimate the ATT, focusing on the effect specifically for those who actually received the discount. The ATT estimates are generally higher, ranging from **16.16 to 23.25**.

> **Key Finding:** The fact that the **ATT (23.25 for 1:1 with replacement)** is substantially higher than the **ATE (15.15 for Regression Adjustment)** suggests **Selection on Returns**. This indicates that the customers who were selected (or selected themselves) to receive the discount responded more strongly to it than the "average" customer in the dataset would have.

---

## 2. Methodological Divergence and Instability

* **The IPW Outlier:** The manual IPW formula yielded an extreme and non-sensical estimate of **-318.12**. This highlights a core issue with standard IPW: it is highly sensitive to propensity scores close to zero or one, which causes weights to explode, leading to massive variance and unstable estimates. The "IPW Weighted Regression (Secondary)" estimate of **15.81** is much more aligned with other methods, likely due to internal stabilization or normalization of weights.
* **Matching Variability:** Matching with replacement (**23.25**) differs significantly from matching without replacement (**17.15**). Matching with replacement generally reduces bias by ensuring treated units are matched to the most similar control units, even if those control units are used multiple times, though it can increase variance.

---

## 3. The Role of Double Robustness and Machine Learning

* **AIPW (18.85):** This estimator is "doubly robust," meaning it remains consistent if either the outcome model (regression) or the selection model (propensity score) is correctly specified. It typically achieves a semiparametric efficiency bound, providing lower asymptotic variance than standard IPW.
* **Double ML (DML):** DML uses flexible ML algorithms to "partial out" or residualize the influence of the 20 covariates.
    * **DML-PLR (XGBoost):** 18.35
    * **DML-PLR (Lasso):** 15.23
    * **DML-PLR (Random Forest):** 13.70

**Observation:** The variation among these shows that the choice of ML algorithm impacts the estimate. XGBoost and AIPW are closely aligned (~18), suggesting they are capturing similar non-linearities in the selection and outcome processes that OLS (15.15) might miss.

---

## 4. Conclusion: Which Estimate to Trust?
The **AIPW** or **Double ML (specifically XGBoost or Lasso)** estimates are generally the most trustworthy for the following reasons:

1.  **Reduction of Model Dependence:** Unlike simple Regression Adjustment, which assumes a strictly linear functional form, DML and AIPW allow for complex, non-linear relationships between the 20 covariates and sales.
2.  **Double Robustness:** AIPW provides two "chances" to get the model right, making it more robust to misspecification of either the treatment assignment or the outcome.
3.  **Stability:** While matching is excellent for reducing model dependence, it often requires discarding units that lack "Common Support," which can change the quantity of interest. AIPW and DML utilize more of the data while maintaining theoretical rigor.

### Final Recommendation
* **Goal: Average Customer Effect** $\rightarrow$ Prioritize the **AIPW estimate (18.85)** or **DML-XGBoost (18.35)**.
* **Goal: Currently Targeted Customers** $\rightarrow$ The **Matching ATT (approx. 23.25)** is more relevant, provided high covariate balance was achieved.